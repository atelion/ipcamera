        ...
        with gr.Blocks(title="Stable Diffusion", css="#vertical_center_align_markdown { position:absolute; top:30%;background-color:white;} .white_background {background-color: #ffffff} .none_border {border: none;border-collapse:collapse;}") as demo:
            with gr.Column():
                with gr.Group():
                    ...
                    with gr.Row():
                        camera_ip = gr.Dropdown(rtsp_streams, value=rtsp_streams[0], label="Select Camera", info="Choose IP addresses",)
                        folder_name_textbox = gr.Textbox(value="Frames", label="Saving Folder", visible=True, interactive=True, type="text")
                    with gr.Row():
                        username_textbox = gr.Textbox(value="admin", label="User", visible=True, interactive=True, type="text")
                        password_textbox = gr.Textbox(value="ADMIN2022", label="Password", visible=True, interactive=True, type="password")
            ...
            def capture_frame(camera_ip, username_textbox, password_textbox):
                self.event = Event()
                rtsp_stream = f"rtsp://{username_textbox}:{password_textbox}@{camera_ip}/Streaming/Channels/101"
                print(f"**************{rtsp_stream}***************")
            ...
            # output_img.change(refresh, outputs=[output_img], queue=False)
            
        demo.launch()
if __name__ == "__main__":
    camera = Camera()    
    # camera.capture_thread.start()    
    # camera.save_screenshot_thread.start()
